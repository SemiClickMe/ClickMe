...
, POST_TIME DATE DEFAULT TO_DATE(SYSDATE, 'YYYY-MM-DD HH24:MI:SS') CONSTRAINT NN_POST_TIME NOT NULL
...
CREATE TABLE IMG_FILE (
    ...
    IMG_TYPE VARCHAR2(30) CONSTRAINT CK_IMG_TYPE CHECK(IMG_TYPE IN ('REPRESENTATIVE','NORMAL')),
    ORIG_PATH VARCHAR2(4000) CONSTRAINT NN_ORIG_PATH NOT NULL,
    THUMB_PATH VARCHAR2(4000) CONSTRAINT NN_THUMB_PATH NOT NULL
);
    ...
    COMMENT ON COLUMN IMG_FILE.IMG_TYPE IS '이미지타입';
    COMMENT ON COLUMN IMG_FILE.ORIG_PATH IS '원본저장경로';
    COMMENT ON COLUMN IMG_FILE.THUMB_PATH IS '썸네일저장경로';
