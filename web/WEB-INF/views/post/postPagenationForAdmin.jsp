...
<c:if test="${ (empty requestScope.postPageCriteria.title) && (empty requestScope.postPageCriteria.content) && (empty requestScope.postPageCriteria.authCode) && (empty requestScope.postPageCriteria.sellerId) && (empty requestScope.postPageCriteria.blindYn) }">
	<c:set var="pageUrl">${ pageContext.servletContext.contextPath }/post/list/admin?currentPage=</c:set>
</c:if>
<c:if test="${ (!empty requestScope.postPageCriteria.title) || (!empty requestScope.postPageCriteria.content) || (!empty requestScope.postPageCriteria.authCode) || (!empty requestScope.postPageCriteria.sellerId) || (!empty requestScope.postPageCriteria.blindYn) }">
	<c:set var="pageUrl">${ pageContext.servletContext.contextPath }/post/list/admin?title=${ requestScope.postPageCriteria.title }&content=${ requestScope.postPageCriteria.content }&authCode=${ requestScope.postPageCriteria.authCode }&sellerId=${ requestScope.postPageCriteria.sellerId }&blindYn=${ requestScope.postPageCriteria.blindYn }&currentPage=</c:set>
</c:if>
<title>타이틀 넣어요</title>
</head>
	<body>
